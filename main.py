...
class PDFMonitorGUI(QMainWindow):
    ...
    def monitor_folder(self, folder: str) -> None:
        ...
        while True:
            try:
                # Obtener la lista de archivos en el nivel superior de la carpeta
                all_entries_in_folder = smbclient.listdir(folder)
                
                # Filtrar solo los archivos PDF
                current_pdfs_in_folder = {os.path.join(folder, entry) for entry in all_entries_in_folder if entry.lower().endswith('.pdf')}
                
                with self.lock:
                    # Identificar los PDFs que ya estaban en nuestra lista para esta carpeta
                    known_pdfs_for_this_folder = {p for p in self.found_pdfs if p.startswith(folder)}
                    
                    # Encontrar nuevos PDFs comparando los conjuntos
                    new_pdfs_paths = current_pdfs_in_folder - known_pdfs_for_this_folder
                    
                    # Añadir los nuevos PDFs a la lista global
                    for doc_path in new_pdfs_paths:
                        if self.wait_doc(doc_path, 2, 5):
                            self.found_pdfs.add(doc_path)
                    
                    # Eliminar de la lista interna los archivos que ya no existen
                    removed_pdfs_paths = known_pdfs_for_this_folder - current_pdfs_in_folder
                    self.found_pdfs.difference_update(removed_pdfs_paths)
